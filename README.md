...
- **rules/**：规则目录。面向工作区整体的规则在此集中，分**具体规则**（R 系列，当前为占位）与**元规则**（M 系列，整合自 lab，见 [`rules/meta/`](rules/meta/README.md)）。**正式具体规则仍在 [`lab/`](lab/README.md) 里实验**，成熟后迁移至此。
...
| 路径 | 内容 | 入口 |
| --- | --- | --- |
| [`rules/`](rules/README.md) | 规则目录：具体规则 R01–R05（占位）+ 元规则 M0–M3（`rules/meta/`，整合自 lab）；正式具体规则源于 lab 实验，未来迁入 | [`rules/README.md`](rules/README.md) |
