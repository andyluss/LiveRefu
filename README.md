...
5. **相对链接深度**：链接必须以「目标文件相对当前文档的正确深度」书写，保证在本地 IDE / 预览器 / 网页版都能跳转。具体规则：
   ...
   - 写法校验：**已由 `pre-commit` 钩子自动拦截**——每次 `git commit` 校验本次暂存的 md 内部链接，失效即阻断并打印明细（启用与跳过方法见 [`tools/hooks/README.md`](tools/hooks/README.md)）。需全量人工检查时仍可运行 `python3 tools/check_links.py`（或 `--sub studio`）。
