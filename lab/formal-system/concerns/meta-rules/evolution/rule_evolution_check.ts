...
// 校验每条元规则（meta-rules/M<编号>-*.md）的演进状态是否符合 M3 状态机。
// 跨运行时 TypeScript（node/deno/bun 直接运行）。与 rule_evolution_check.py 同功能。
...
const META_DIR = path.normalize(path.join(HERE, "..")); // meta-rules/
