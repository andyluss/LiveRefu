...
- **M1**：`lab/formal-system` 是一个**关注点**（实验）；根有出入口 [`README.md`](../../README.md)。
  子项 = **子关注点统一目录 [`concerns/`](../) + 类型目录 + 根文档**（`CHANGELOG.md`、`EXPERIMENT.md`），无散落文件；子关注点按 [M1](M1-file-organization.md) 递归套用（如 `meta-rules/evolution/`、`visual-fallback/evolution-history/`）。
...
`python3 lab/formal-system/concerns/meta-rules/tools/meta_rules_check.py` → 退出码 0 = 结构合法（实测 28 合规 / 0 违规）。
