...
> 本文说明本仓库用**两级闸门**校验 Markdown 相对链接：本地 pre-commit 钩子（提交前）+ CI 链接检查（提交后兜底）。工具脚本见 [`../tools/check_links.py`](../tools/check_links.py)、[`../tools/install_hooks.sh`](../tools/install_hooks.sh)、[`../tools/hooks/pre-commit`](../tools/hooks/pre-commit)。
...
- 校验逻辑复用 [`tools/check_links.py`](../tools/check_links.py)：跳过外链、纯锚点、无目标条目；只判定真正需要存在的内部相对链接。
